...
https://github.com/albertescanes/machined-factory/releases/download/nightly/centos.$(arch).tar.xz centos
```
...
## Available images
The following images are available: `arch`, `centos`, `debian`, `fedora`, and `rhel-ubi`.

All images are built for both `x86_64` and `aarch64` except `arch` (`x86_64` only).
